Public Class Form_ModificaImballo
    ...
    Private Sub Bt_LoadImage_Click(sender As Object, e As EventArgs) Handles Bt_LoadImage.Click
        Dim LoadImage As New OpenFileDialog With {.Filter = "File immagine | *.jpg; *.jpeg; *.jpe; *.jfif; *.png; *.bmp; *.gif|All files (*.*)|*.*"}
        ...
        End If
    End Sub
    Private Sub Bt_SalvaVecchio_Click(sender As Object, e As EventArgs) ' Handles Bt_Salva.Click
    ...
    Private Sub Bt_SalvaNew_Click(sender As Object, e As EventArgs) Handles Bt_Salva.Click
        If Form_ModificaImballo_CosaModificareDialog.ShowDialog = MsgBoxResult.Ok Then

            Dim IR As ModPackDBDataSet.ImballiRow = CType(ImballiBindingSource.Current, DataRowView).Row

            'Salva i vecchi per fare confronto
            Dim VecchiM2 As Single = IR.M2
            Dim VecchiM3 As Single = IR.M3
            Dim PrezzoVecchio As Single = IR.Prezzo
            ...
            Try
                ...
                'Ricalcola m3, m2 e prezzo se necessario
                With Form_ModificaImballo_CosaModificareDialog

                    Select Case IR.Tipo
                        Case "GDA"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3(IR.Imballo)
                            If .Ric_Prz = True Then IR.Prezzo = Prezzi.Base(IR.Tipo, IR.M3, IR.M2, IR.Tipo_Rivestimento, IR.HT)
                            LOG.Write("Ricalcolato prezzo " & IR.Imballo & " - Vecchio: €" & PrezzoVecchio & " Nuovo: €" & IR.Prezzo)
                        Case "G"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3(IR.Imballo)
                        ...
                        Case "GST"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3(IR.Imballo)
                            If .Ric_Prz = True Then IR.Prezzo = Prezzi.GST(IR.M3, IR.L, IR.P, IR.H, IR.Tipo_Rivestimento, IR.HT)
                            LOG.Write("Ricalcolato prezzo " & IR.Imballo & " - Vecchio: €" & PrezzoVecchio & " Nuovo: €" & IR.Prezzo)
                        Case "C"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3_Morali(IR.Imballo)
                            If .Ric_Prz = True Then IR.Prezzo = Prezzi.C(IR.M3, IR.L, IR.P, IR.H, IR.Tipo)
                            LOG.Write("Ricalcolato prezzo " & IR.Imballo & " - Vecchio: €" & PrezzoVecchio & " Nuovo: €" & IR.Prezzo)
                        Case "CL"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3_Morali(IR.Imballo)
                        ...
                        Case "GRO"
                            If .Ric_M2 = True Then IR.M2 = Imballo.Ricalcolo_M2(IR.Imballo)
                            If .Ric_M3 = True Then IR.M3 = Imballo.Ricalcolo_M3_Morali(IR.Imballo)
                            If .Ric_Prz = True Then IR.Prezzo = Prezzi.Base(IR.M3, IR.L, IR.P, IR.H, IR.Tipo)
                            LOG.Write("Ricalcolato prezzo " & IR.Imballo & " - Vecchio: €" & PrezzoVecchio & " Nuovo: €" & IR.Prezzo)
                        Case Else
                            MsgBox("Tipo non riconosciuto")
                    End Select
                    ...
                    Dim Stringa As String = " ------ MODIFICA " & IR.Imballo & " ------ " & vbCrLf
                    If VecchiM2 <> IR.M2 Then Stringa += " -- Vecchi m2 = " & VecchiM2 & " - Nuovi m2 = " & IR.M2 & " --" & vbCrLf
                    If VecchiM3 <> IR.M3 = True Then Stringa += " -- Vecchi m3 = " & VecchiM3 & " - Nuovi m3 = " & IR.M3 & " --" & vbCrLf
                    If PrezzoVecchio <> IR.Prezzo = True Then Stringa += " -- Vecchi € = " & VecchiM2 & " - Nuovi € = " & IR.M2 & " --" & vbCrLf
